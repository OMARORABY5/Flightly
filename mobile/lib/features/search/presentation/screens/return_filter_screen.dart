// return_filter_screen.dart
// Filter screen for the return leg of a round-trip search.
// Functionally identical to FilterScreen but targets returnFilterOptionsProvider
// and ReturnFlightResultsNotifier instead of the outbound providers.
...
class ReturnFilterScreen extends ConsumerStatefulWidget {
  const ReturnFilterScreen({Key? key}) : super(key: key);

  @override
  ConsumerState<ReturnFilterScreen> createState() => _ReturnFilterScreenState();
}

class _ReturnFilterScreenState extends ConsumerState<ReturnFilterScreen> {
  ...
  void _applyFilters() {
    final filters = ref.read(returnFilterOptionsProvider);
    ref.read(returnFilterOptionsProvider.notifier).state = filters.copyWith(
      maxPrice: _maxPrice ?? -1.0,
      maxStops: _maxStops ?? -1,
  ...
  void _resetFilters() {
    ...
    ref.read(returnFilterOptionsProvider.notifier).state = const FilterOptions();
    ref.read(returnFlightResultsProvider.notifier).searchFlights();
  }
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text('Return Filters', style: AppTextStyles.headingMedium),
                    TextButton(
                  ...
                  child: Column(
                    ...
                    children: [
                        ...
                        child: Column(
                          children: [
                            SliderTheme(
                              ...
                              child: Slider(
                                value: _maxDuration?.toDouble() ?? 1440.0,
                                min: 60,
                                max: 1440,
                                divisions: 23,
                      ...
                      airlinesAsync.when(
                        data: (airlines) {
                          if (airlines.isEmpty) {
                            return Text('No airlines available.', style: AppTextStyles.bodyMedium.copyWith(color: AppColors.textSecondary));
                          }
                          return GlassCard(
                        ...
                        error: (err, stack) => Text('Error: $err', style: AppTextStyles.bodySmall.copyWith(color: AppColors.error)),
                      ),
                    ],
                  ),
                ),
